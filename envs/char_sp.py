...
class EnvDataset(Dataset):
    ...
    def __init__(self, env, task, train, rng, params, path):
        ...
        self.count = 0
        self.conditioned = params.is_dataset_conditioned

        self.random_eval = params.random_eval
                ...
                if not train:
                    ...
                    for i, line in enumerate(f):
                        ...
                        if i % params.n_gpu_per_node == params.local_rank:
                            lines.append(line.rstrip().split('|'))
            print("Loop done")
            self.data_1 = [xy.split('\t') for _, xy in lines]
            if self.conditioned:
                self.data = [xy for xy in self.data_1 if len(xy) == 4]
            else:
                self.data = [xy for xy in self.data_1 if len(xy) == 2]
            print("Data saved")
        ...
        if self.train:
            ...
            self.size = params.max_test_size
            if path is not None:
                if len(self.data) < params.max_test_size:
                    self.size = len(self.data)
            # self.size = params.max_test_size if path is None else len(self.data)
    ...
    def collate_fn(self, elements):
        ...
        """
        if self.conditioned:
            x, y, func, step, cond = zip(*elements)
        else:
            x, y = zip(*elements)
        ...
        func = [torch.LongTensor([int(el) for el in seq]) for seq in func]
        step = [torch.LongTensor([int(el) for el in seq]) for seq in step]
        cond = [torch.LongTensor([int(el) for el in seq]) for seq in cond]
        func, func_len = self.env.batch_sequences(func)
        step, step_len = self.env.batch_sequences(step)
        cond, cond_len = self.env.batch_sequences(cond)

        return (x, x_len), (y, y_len), (func, func_len), (step, step_len), (cond, cond_len), torch.LongTensor(nb_ops)
    ...
    def read_sample(self, index):
        ...
        Read a sample.
        """
        if self.train or self.random_eval: # Randomly get items
        ...
        if not self.conditioned:
            ...
            x, y, func, steps = self.data[index]
            x = x.split()
            y = y.split()
            func = func.split()
            step = steps.split()
            conditioning =  step + func[1:]
            return x, y, func, step, conditioning
